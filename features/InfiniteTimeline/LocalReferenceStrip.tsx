...
} from '@/features/InfiniteTimeline/helpers';
import {
  TIMELINE_EDGE_PULL_MAX,
  TIMELINE_EDGE_PULL_TRIGGER,
} from '@/features/InfiniteTimeline/constants';
...
type LocalReferenceStripProps = {
  ...
  timezone: string;
  width: number;
  onUserInteraction?: (gestureId: number) => void;
  onScrollSettled?: (focusedHourIndex: number, gestureId: number) => void;
...
function LocalReferenceStripComponent({
  ...
  const activeGestureId = useSharedValue(0);
  ...
  const sideDates = useMemo(() => {
    const previousDay = new Date(anchorDay);
    previousDay.setDate(previousDay.getDate() - 1);

    const nextDay = new Date(anchorDay);
  ...
  const pan = useMemo(
    () =>
      Gesture.Pan()
        ...
        .onBegin(() => {
          cancelAnimation(x);
          cancelAnimation(edgePull);
          gestureSessionId.value += 1;
          activeGestureId.value = gestureSessionId.value;

          if (onUserInteraction) {
            runOnJS(onUserInteraction)(activeGestureId.value);
          }
        ...
        .onEnd((event) => {
          ...
          const completedGestureId = activeGestureId.value;

          if (onScrollSettled) {
            runOnJS(onScrollSettled)(focusedHourIndex, completedGestureId);
          }

          x.value = withSpring(targetOffset, { duration: 220 });
        }),
    [
      activeGestureId,
      edgePull,
...
export const LocalReferenceStrip = React.memo(
  ...
  (prevProps, nextProps) =>
    prevProps.anchorDay.getTime() === nextProps.anchorDay.getTime() &&
    prevProps.gestureSessionId === nextProps.gestureSessionId &&
