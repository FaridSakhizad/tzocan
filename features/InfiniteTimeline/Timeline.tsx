...
import { HourStrip } from '@/features/InfiniteTimeline/HourStrip';
import { LocalReferenceStrip } from '@/features/InfiniteTimeline/LocalReferenceStrip';
...
export default function TimelineScreen() {
  ...
  const isInfiniteTimelineEnabled = true;

  const [nowMs, setNowMs] = useState(Date.now());
  const [selectedDayState, setSelectedDayState] = useState(() => getLocalDayStart(new Date()));
  ...
  const [windowCenterHourIndex, setWindowCenterHourIndex] = useState(initialFocusedHourIndexRef.current);
  ...
  const latestGestureIdRef = useRef(0);
  const focusUpdateSourceRef = useRef<'gesture' | 'programmatic'>('programmatic');
  ...
  const selectedDay = useMemo(
    () => isInfiniteTimelineEnabled ? getLocalDayStart(focusedDateTime) : selectedDayState,
    [focusedDateTime, isInfiniteTimelineEnabled, selectedDayState]
  );
  ...
  useEffect(() => {
    ...
    if (nextToday.getTime() !== previousToday.getTime() && !isDayTransitioningRef.current) {
      if (isInfiniteTimelineEnabled) {
        if (focusedHourIndex === previousCurrentHourIndex) {
          focusUpdateSourceRef.current = 'programmatic';
          setFocusedHourIndex(nextCurrentHourIndex);
          setWindowCenterHourIndex(nextCurrentHourIndex);
        } else if (focusedHourWasOnPreviousToday) {
          const offsetWithinDay = focusedHourIndex - previousTodayHourIndex;
          const nextFocusedHourIndex = getHourIndexForDate(nextToday) + offsetWithinDay;
          focusUpdateSourceRef.current = 'programmatic';
          setFocusedHourIndex(nextFocusedHourIndex);
          setWindowCenterHourIndex(nextFocusedHourIndex);
        }
      } else if (selectedDayState.getTime() === previousToday.getTime()) {
        setSelectedDayState(nextToday);
  ...
  }, [focusedHourIndex, isInfiniteTimelineEnabled, nowDate, selectedDayState]);
  ...
  const hourIndices = useMemo(
    () => isInfiniteTimelineEnabled ? getTimelineHourIndicesAroundHour(windowCenterHourIndex) : getTimelineHourIndicesForDay(selectedDay),
    [isInfiniteTimelineEnabled, selectedDay, windowCenterHourIndex]
  );
  const startHourIndex = hourIndices[0] ?? (isInfiniteTimelineEnabled ? windowCenterHourIndex : focusedHourIndex);
  const endHourIndex = hourIndices[hourIndices.length - 1] ?? (isInfiniteTimelineEnabled ? windowCenterHourIndex : focusedHourIndex);
  ...
  const firstFocusableHourIndex = hourIndices[0] ?? (isInfiniteTimelineEnabled ? windowCenterHourIndex : focusedHourIndex);
  const lastFocusableHourIndex = hourIndices[hourIndices.length - 1] ?? (isInfiniteTimelineEnabled ? windowCenterHourIndex : focusedHourIndex);
  ...
  const shouldRecenterWindow = useCallback((hourIndex: number) => {
    const thresholdHours = TIMELINE_DAY_HOURS * 2;

    return (
      hourIndex <= startHourIndex + thresholdHours ||
      hourIndex >= endHourIndex - thresholdHours
    );
  ...
  useLayoutEffect(() => {
    ...
    if (isInfiniteTimelineEnabled && focusUpdateSourceRef.current === 'gesture') {
      focusUpdateSourceRef.current = 'programmatic';
      return;
    }

    applyScrollOffsetForHourIndex(focusedHourIndex, false);
  }, [applyScrollOffsetForHourIndex, focusedHourIndex, isInfiniteTimelineEnabled, selectedDay, windowCenterHourIndex]);
  ...
  const handleTimelineInteraction = useCallback((gestureId: number) => {
    latestGestureIdRef.current = gestureId;
  }, []);

  const handleTimelineScrollSettled = useCallback((nextFocusedHourIndex: number, gestureId: number) => {
    if (gestureId !== latestGestureIdRef.current) {
      return;
    }

    if (isInfiniteTimelineEnabled) {
      focusUpdateSourceRef.current = 'gesture';

      if (shouldRecenterWindow(nextFocusedHourIndex)) {
        const deltaHours = nextFocusedHourIndex - windowCenterHourIndex;
        x.value -= deltaHours * TIMELINE_CELL_WIDTH;
        setWindowCenterHourIndex(nextFocusedHourIndex);
      }
  ...
  }, [firstFocusableHourIndex, isInfiniteTimelineEnabled, lastFocusableHourIndex, shouldRecenterWindow]);
  ...
  const handleResetTimeline = useCallback(() => {
    runDayTransition(() => {
      const nextFocusedHourIndex = getHourIndexForDate(new Date());
      focusUpdateSourceRef.current = 'programmatic';
      setFocusedHourIndex(nextFocusedHourIndex);
      ...
      if (isInfiniteTimelineEnabled) {
        ...
        setSelectedDayState(getLocalDayStart(new Date()));
      }
    });
  }, [isInfiniteTimelineEnabled, runDayTransition]);
  ...
  const shiftDayBy = useCallback((offsetDays: number) => {
    runDayTransition(() => {
      ...
      focusUpdateSourceRef.current = 'programmatic';
      setFocusedHourIndex(nextFocusedHourIndex);
      if (isInfiniteTimelineEnabled && shouldRecenterWindow(nextFocusedHourIndex)) {
        setWindowCenterHourIndex(nextFocusedHourIndex);
      }
  ...
  }, [focusedHourIndex, isInfiniteTimelineEnabled, runDayTransition, selectedDay, shouldRecenterWindow]);
  ...
  }, [isInfiniteTimelineEnabled, runDayTransition, selectedDay, shouldRecenterWindow]);
  ...
  const handlePickerDayChange = useCallback(
    ...
    [focusedHourIndex, isInfiniteTimelineEnabled, runDayTransition, selectedDay]
  );
  ...
  }, [focusedHourIndex, isInfiniteTimelineEnabled, pickerDraftDay, runDayTransition, selectedDay]);
